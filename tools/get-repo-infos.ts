 ...
 */
async function fetchLatestReleaseDate(owner: string, repo: string): Promise<string | undefined> {
  try {
      ...
      console.error(`Error fetching latest release date for ${owner}/${repo}:`, error.message);
      return;
  }
}
...
async function fetchRepoInfo(
  ...
  try {
    ...
    const lastPublishedDateIso = await fetchLatestReleaseDate(owner, repo) as string;
